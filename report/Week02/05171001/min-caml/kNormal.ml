...
let rec string_list_to_string string list =
  match list with
  | [] -> string
  | y::ys -> string_list_to_string (string ^ y ^ "  ") ys

let rec print_knormal indent e =
  match e with
  ...
  | IfEq(s1, s2, e1, e2) ->
    ...
    print_knormal (indent ^ "  ") e1;
    print_string (indent ^ "ELSE\n");
    print_knormal (indent ^ "  ") e2
  | IfLE(s1, s2, e1, e2) ->
  ...
  | App(e, es) ->
    print_string (indent ^ "APP " ^ e ^ " " ^ (string_list_to_string "" es) ^ "\n")
  | Tuple(es) ->
    print_string (indent ^ "TUPLE " ^ (string_list_to_string "" es) ^ "\n")
  ...
  | ExtFunApp(e, es) ->
    print_string (indent ^ "EXTFUNAPP "^ e ^ " " ^ (string_list_to_string "" es) ^ "\n")
...
let rec delete_duplication list e =
  ...
  | Let((x, t), e1, e2) ->
    (try
       ...
       print_string "***********Duplication detected***********\n";
       print_knormal "  " e1;
       print_string "***********is a duplication***************\n";
